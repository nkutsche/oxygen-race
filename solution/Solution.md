...
Press `ctrl` and click on the string, repressenting the key name (`'key'`). You will jump to the declaration of the key.

Special feature: The `$key-name` paremter of the `key` function is not an usual reference to the used key, because the key function expects a string value for the key name. This means, that the key name can also be defined by another XPath expression. It is just common practice to specifiy a fix string for the `$key-name` parameter. The Oxygen has an exceptional implementation to recognice such key references. This works only if the `$key-name` parameter is a fix string. 
...
In the tool bar of the Oxygen you will finde a button with an arrow in left direction. The name of the button is "back". You can use it to find the last cursor positions.

This button is for shorter go-backs very helpfull, but please note, that this button is sometimes a bit buggy. I had the experiance, that it was to sensitive and I had to click it too often, but also that I jumped somewhere, where I didn't expected it. 
